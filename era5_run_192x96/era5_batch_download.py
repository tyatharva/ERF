...
def main():
    ...
    def fetch_batch(stream, spec, tsteps, tag, cs, ce):
        ...
        # CDS day/time lists are a CROSS PRODUCT, so a chunk that does not fill
        # every (day, time) slot gets back MORE timesteps than it asked for and
        # len(tsteps) is the wrong expectation. A 9-frame chunk (Jan 9 00..21Z
        # plus Jan 10 00Z) is 2 days x 8 times = 16 timesteps: measured 7104
        # messages against a len(tsteps) expectation of 3996, and the validator
        # deleted a perfectly good download. Expect what download_batch actually
        # requests. split_batch buckets by validity time and drops the surplus.
        ndays = len({t.strftime("%d") for t in tsteps})
        ntimes = len({t.strftime("%H:%M") for t in tsteps})
        expected = ndays * ntimes * spec["fields_per_time"]
